...
- Produces: `export const script: Script` —— 全部剧情节点（19 个，含 QTE 两个结果节点）；节点 ID 与 spec §3 表格一一对应
